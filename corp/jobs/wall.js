...
function fillWall()
{
	console.log("calling fillwall in corp wall");
	cid = $("#cidval").val();
...
function wallInfo()
{
	if(xhr.readyState == 4 && xhr.status == 200)
	{
		...
		else
		{
			...
			{
				if(count[i] > 1)
					updates += '<p class="newupdate">' + '<a href="http://localhost/corp/jobs/applicants.php?jid=' + jids[i] + '">' + count[i] + "</a> applicants for " + '<a href="http://localhost/corp/jobs/Specificjob.php?jid=' + jids[i] + '">' + titles[i] + "</a></p>"
				else if(count[i] == 1)
					updates += '<p class="newupdate">' + '<a href="http://localhost/corp/jobs/applicants.php?jid=' + jids[i] + '">' + count[i] + "</a> applicant for " + '<a href="http://localhost/corp/jobs/Specificjob.php?jid=' + jids[i] + '">' + titles[i] + "</a></p>"
			}
